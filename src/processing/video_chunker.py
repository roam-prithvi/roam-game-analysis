...
def calculate_chunk_times(
    total_duration: float, chunk_duration: int, overlap_seconds: int
...
def create_video_chunks(
    ...
    try:
        ...
        chunk_times = calculate_chunk_times(
            total_duration, chunk_duration, overlap_seconds
        )
...
def chunk_session(
    ...
    Args:
        ...
        force: If True, re-chunk even if chunks already exist

    Returns:
        True if successful, False otherwise
    """
    # Look for the trimmed video first, fallback to original
    video_file = session_path / "trimmed_screen_recording.mp4"
    ...
    # Skip if already chunked (unless force is True)
    if output_dir.exists() and (output_dir / "metadata.json").exists():
        if not force:
            logger.info(f"Session already chunked: {session_path}")
            return True
        else:
            logger.info(f"Force re-chunking: removing existing chunks in {output_dir}")
            shutil.rmtree(output_dir)

    logger.info(f"Chunking video: {video_file}")
    result = create_video_chunks(
        video_file, output_dir, chunk_duration, overlap_seconds
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Create video chunks for spatial understanding pipeline"
    )
    parser.add_argument("--game", type=str, help="Game name (e.g., 'subway surfers')")
    parser.add_argument(
        "--session", type=str, help="Specific session timestamp to chunk"
    )
    parser.add_argument(
        "--all", action="store_true", help="Chunk all sessions for the game"
    )
    parser.add_argument(
        "--chunk-duration",
        type=int,
        default=5,
        help="Duration of each chunk in seconds (default: 5)",
    )
    parser.add_argument(
        "--overlap",
        type=int,
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Force re-chunking even if chunks already exist",
    )
    ...
    if args.all:
        ...
        for session in sessions:
            if chunk_session(session, args.chunk_duration, args.overlap, args.force):
                success_count += 1
        ...
        if chunk_session(session_path, args.chunk_duration, args.overlap, args.force):
        ...
        if chunk_session(latest_session, args.chunk_duration, args.overlap, args.force):
            logger.info("Chunking completed successfully")
        else:
            logger.error("Chunking failed")
