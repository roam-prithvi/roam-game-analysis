...
class UniversalGroundedSAM2Pipeline:
    ...
    def __init__(self, session_path: Path, max_frames: int = 100, frame_interval: int = 30, 
    ...
    def extract_frames(self) -> List[Path]:
        ...
        try:
            ...
            frames_to_extract = min(self.max_frames, total_frames // self.frame_interval)
    ...
    def create_universal_prompt(self, frame_idx: int = 0) -> str:
        ...
        if self.gemini_objects:
            prompts.update(list(self.gemini_objects)[:15])  # Limit to avoid overly long prompts
        
        # Strategy 2: Add universal game object prompts
        universal_prompts = [
            "character", "player", "enemy", "monster", "opponent",
            "obstacle", "barrier", "wall", "platform", "block",
            "collectible", "coin", "gem", "item", "powerup",
            "button", "menu", "icon", "score", "health",
            "vehicle", "car", "train", "ship", "plane",
            "weapon", "tool", "door", "key", "chest"
        ]
    ...
    def run_grounded_sam2_on_frame(self, frame_path: Path, prompt: str) -> Dict:
        ...
        try:
            ...
            cmd = [
                ...
                "--text-prompt", prompt,
                "--img-path", str(abs_frame_path),
                "--output-dir", str(abs_output_dir),
                "--box-threshold", "0.25",  # Lower threshold for more detections
                "--text-threshold", "0.20"
            ]
            
            print(f"🔍 Running detection on {frame_path.name} with prompt: {prompt[:100]}...")
            
            # Debug: Print the exact command being run
            print(f"  Command: {' '.join(cmd)}")
            
            # Use absolute path for cwd as well
            result = subprocess.run(cmd, capture_output=True, text=True, cwd=str(grounded_sam_dir.absolute()))
            ...
            if result.returncode != 0:
                ...
                if temp_link_path and (temp_link_path.exists() or temp_link_path.is_symlink()):
                    temp_link_path.unlink()
            ...
            if results_file.exists():
                ...
                print(f"✅ Found {len(detection_results.get('annotations', []))} objects in {frame_path.name}")
    ...
    def categorize_detections(self, detections: List[Dict]) -> Dict[str, List[Dict]]:
        ...
        for detection in detections:
            ...
            if any(keyword in class_name for keyword in ["character", "player", "hero", "avatar"]):
                category = "player"
            elif any(keyword in class_name for keyword in ["enemy", "monster", "opponent", "zombie", "alien"]):
                category = "enemies"
            elif any(keyword in class_name for keyword in ["obstacle", "barrier", "wall", "block", "train", "car"]):
                category = "obstacles"
            elif any(keyword in class_name for keyword in ["coin", "gem", "collectible", "item", "powerup", "bonus"]):
                category = "collectibles"
            elif any(keyword in class_name for keyword in ["button", "menu", "icon", "score", "health", "timer"]):
                category = "ui"
            elif any(keyword in class_name for keyword in ["building", "tree", "platform", "ground", "background"]):
                category = "environment"
            elif any(keyword in class_name for keyword in ["door", "lever", "switch", "tool", "weapon", "key"]):
    ...
    def save_frame_analysis(self, frame_results: List[Dict], objectives_prompt: Optional[str] = None) -> str:
        ...
        summary = {
            ...
            "categories_found": len([c for c, count in category_totals.items() if count > 0]),
    ...
    async def run_complete_pipeline(self) -> str:
        ...
        try:
            ...
            total_detections = sum(
                ...
                for r in frame_results if r.get("success")
            )
...
Examples:
  ...
  python universal_grounded_sam2_pipeline.py data/GameName/session_folder --no-objectives
    ...
    if not (session_path / "screen_recording.mp4").exists():
        print(f"❌ Video file not found: {session_path / 'screen_recording.mp4'}")
        return
