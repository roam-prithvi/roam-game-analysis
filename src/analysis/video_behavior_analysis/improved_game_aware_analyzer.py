...
from .game_prompts import get_game_prompts, get_primary_game_prompts, get_supported_games
...
class ImprovedGameAwareAnalyzer:
    ...
    def __init__(self, 
        ...
        self.analysis_dir = self.session_path / "analysis" / f"{detector_type}_game_aware"
    ...
    def _detect_game_name(self) -> str:
        ...
        path_parts = [p.name.lower() for p in self.session_path.parents] + [self.session_path.name.lower()]
    ...
    def categorize_detections(self, detections: List[Detection]) -> Dict[str, List[Detection]]:
        ...
        for det in detections:
            ...
            # Player detection
            if any(word in class_name for word in ['character', 'player', 'person', 'hero']):
                categories['player'].append(det)
                categorized = True
            
            # Enemy detection
            elif any(word in class_name for word in ['zombie', 'enemy', 'monster', 'opponent']):
                categories['enemies'].append(det)
                categorized = True
            
            # Obstacle detection
            elif any(word in class_name for word in ['obstacle', 'barrier', 'train', 'car', 'truck']):
                categories['obstacles'].append(det)
                categorized = True
            
            # Collectible detection
            elif any(word in class_name for word in ['coin', 'gem', 'collectible', 'power', 'bonus']):
                categories['collectibles'].append(det)
                categorized = True
            
            # UI elements
            elif any(word in class_name for word in ['button', 'menu', 'score', 'ui', 'interface']):
                categories['ui'].append(det)
                categorized = True
            
            # Environment
            elif any(word in class_name for word in ['background', 'platform', 'ground', 'building']):
    ...
    def merge_similar_detections(self, 
    ...
    def save_detection_cutouts(self, 
                             frame_path: str, 
            ...
            for i, det in enumerate(detections):
                try:
                    if det.mask is not None:
                        ...
                        if self._extract_cutout_with_mask(frame_path, det.mask, str(output_path)):
                            cutout_info.append({
                                'category': category,
                                'class_name': det.class_name,
                        ...
                        if self._extract_cutout_with_bbox(frame_path, det.bbox, str(output_path)):
                            cutout_info.append({
                                'category': category,
                                'class_name': det.class_name,
                                'confidence': det.confidence,
    ...
    def _extract_cutout_with_mask(self, image_path: str, mask: np.ndarray, output_path: str) -> bool:
        ...
        try:
            ...
            print(f"🔧 Debug mask - shape: {mask.shape}, dtype: {mask.dtype}, range: {mask.min()}-{mask.max()}")
            ...
            if mask.dtype == bool:
                ...
                if mask.max() <= 1:
                    ...
                    print(f"⚠️ Mask appears to be in low range (max={mask.max()}), scaling to 0-255")
                    mask = (mask / mask.max() * 255).astype(np.uint8)
            
            # Final check - ensure mask has meaningful values
            if mask.max() == 0:
                print(f"⚠️ Mask has no positive values after processing, skipping cutout")
                return False
            ...
            print(f"🔧 Final alpha - min: {final_alpha.min()}, max: {final_alpha.max()}, "
            ...
            if non_transparent_pct < 0.01:  # Less than 0.01% of image
                print(f"⚠️ Cutout too small ({non_transparent_pct:.3f}%), likely noise - skipping")
                return False
    ...
    def _extract_cutout_with_bbox(self, image_path: str, bbox: List[float], output_path: str) -> bool:
    ...
    async def process_frame(self, frame_path: str, frame_idx: int) -> Dict[str, Any]:
        ...
        print(f"📂 Categorized into {sum(1 for cats in categorized.values() if cats)} categories")
    ...
    async def analyze_video(self, max_frames: int = 1000, frame_interval: int = 1):
        ...
        if frame_interval == 1:
            ...
            print(f"📌 Processing {len(sampled_frames)} sampled frames (every {frame_interval}th frame)")
        ...
        for i in range(0, len(sampled_frames), batch_size):
            ...
            print(f"⏳ Completed batch {i//batch_size + 1}/{(len(sampled_frames) + batch_size - 1)//batch_size}")
        ...
        output_file = self.analysis_dir / f"improved_game_aware_analysis_{self.detector_type}.json"
        ...
        for category, count in sorted(overall_categories.items(), key=lambda x: x[1], reverse=True):
...
async def main():
    ...
    parser.add_argument("--detector", choices=["grounded_sam", "yolo"], 
                       default="grounded_sam", help="Detector type to use")
    ...
    if args.path:
        ...
        if args.all_sessions and path.is_dir():
            ...
            print(f"📁 Processing {len(sessions)} sessions with {args.detector} detector")
        ...
        game_name = input("\n🎮 Enter game name (or press Enter to auto-detect): ").strip()
        if not game_name:
            game_name = None
            
        detector_type = input("🔧 Choose detector [grounded_sam/yolo] (default: grounded_sam): ").strip()
